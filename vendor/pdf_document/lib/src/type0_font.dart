...
class Type0Font {
  ...
  /// The content code the document's own font already draws [rune] with (the
  /// inverse of `/ToUnicode`), or null when no shown glyph maps to it.
  ///
  /// Subsetted embedded programs - notably the pdf package's - often carry an
  /// empty/limited `cmap`, so [glyphForRune] reports 0 even for characters the
  /// font physically draws. `/ToUnicode` is the authoritative record of what
  /// the document font can actually represent, so a rune present there can be
  /// re-emitted as its existing code: identity and in-subset replacements then
  /// stay in the document font instead of dragging in a fallback face.
  int? codeForRune(int rune) {
    final match = _stringOf(rune);
    int? best;
    for (final entry in codeToText.entries) {
  ...
  /// Whether the document font can draw [rune]: it has a shown code for it,
  /// the embedded program's own cmap maps it to a real glyph, or it is a
  /// space (always encodable as a bare advance - no glyph outline needed).
  bool canDraw(int rune) =>
      rune == 0x20 || codeForRune(rune) != null || glyphForRune(rune) != 0;

  /// The content code to emit for [rune]: the document font's own code when it
  /// has one, else the embedded program's glyph id. 0 for a space the font
  /// never draws (callers advance past it instead of emitting a glyph).
  int codeFor(int rune) => codeForRune(rune) ?? glyphForRune(rune);

  /// The advance of a space in thousandths of an em: the document font's own
  /// space metric when it draws one, else the embedded program's, else the
  /// standard quarter-em space.
  ///
  /// Subset producers (notably the pdf package) render inter-word gaps with
  /// positioning operators rather than a space glyph, so their embedded
  /// program and `/ToUnicode` carry no space at all. A replacement containing
  /// spaces must still stay in the document font - emitting a space as a bare
  /// advance keeps the word gap without dragging the whole edit into a
  /// fallback face.
  double spaceWidth() {
    final code = codeForRune(0x20);
    if (code != null) return widthOf(code);
    final gid = glyphForRune(0x20);
    if (gid != 0) return _cidWidths[gid] ?? advanceForGlyph(gid);
    return 250; // PostScript default space (matches [PdfUnicodeFont])
  }
