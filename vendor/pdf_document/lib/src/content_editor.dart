...
extension PdfContentEditing on PdfEditor {
  ...
  int _replaceText(
    ...
    // the text object (BT..ET) enclosing the current operation, for widening a
    // Type0 run to the whole object the producer positioned. The object's
    // operators are buffered ([textObjOps]) until the object resolves - a
    // widened Type0 object is spliced whole (its prefix operators must not be
    // emitted twice), anything else is flushed op by op.
    var textObjStart = -1;
    var textObjEnd = -1;
    final textObjOps = <ContentOperation>[];

    void emit(ContentOperation o) {
      if (textObjStart >= 0) {
        textObjOps.add(o);
      } else {
        rewritten.add(o);
    ...
    while (i < ops.length) {
      ...
      if (op.operator == 'BT') {
        textObjStart = i;
        textObjEnd = ops.indexWhere((o) => o.operator == 'ET', i + 1);
        textObjOps.clear();
        if (textObjEnd < 0) textObjStart = -1; // no closing ET - don't buffer
      } else if (op.operator == 'ET') {
        // a text object that was never widened - emit its buffered operators
        rewritten.addAll(textObjOps);
        textObjOps.clear();
        textObjStart = -1;
        textObjEnd = -1;
      }
      ...
      if (op.operator == 'Tj' || op.operator == 'TJ') {
        ...
        // read as one line and may be merged into a single TJ. A /Type0
        // text object is positioned as a unit instead - per-glyph `Td`,
        // RTL /ReversedChars, /ActualText spans - so it is rewritten
        // whole, in place.
        final type0Object = textObjStart >= 0 &&
            textObjEnd > textObjStart &&
            operationRange == null &&
            _isType0(cos, font);
        var run = <ContentOperation>[];
        if (type0Object) {
          run.addAll(ops.sublist(textObjStart, textObjEnd + 1));
          textObjOps.clear(); // the whole object is spliced, prefix included
          i = textObjEnd + 1;
        } else {
          // flush whatever buffered operators precede this show
          rewritten.addAll(textObjOps);
          textObjOps.clear();
          while (i < ops.length &&
    ...
    if (count > 0) {      // write the new glyphs' widths and Unicode values into the composite
...
class _SimpleRunCodec extends RunCodec {
  ...
  List<RunItem> flatten(List<ContentOperation> run) =>
      [for (final c in simpleRunCells(run)) CellItem(c)];
